...
int16 g_nGPSBufferReadPtr=0;   	//Read (decode) pointer
int16 g_nGPSBufferLogPtr=0;		//File Log pointer
...
#define GPS_INC_LOGPTR(n)   (g_nGPSBufferLogPtr = (g_nGPSBufferLogPtr + (n))%GPS_BUFFER_LEN)
#define GPS_INC_WRITEPTR(n)   (g_nGPSBufferWritePtr = (g_nGPSBufferWritePtr + (n))%GPS_BUFFER_LEN)
#define GPS_IS_BUFFER_EMPTY	(g_nGPSBufferReadPtr == g_nGPSBufferWritePtr)
#define GPS_LOG_GET_CHAR	(GPSBuffer[g_nGPSBufferLogPtr])
...
void GPS_isr()
{
	...
	ch = fgetc(GPSDEV);   //Get one char from GPS
	if(g_bForwardGPS)
		fputc(ch, MDLDRV);

	//Copy char to GPSBuffer for sending out   
	GPSBuffer[g_nGPSBufferWritePtr] = ch;
	GPS_INC_WRITEPTR(1);

#ifdef LED_GPS
	output_high(LED_GPS);
	g_nGPSSignalTick = 20;	//200ms
#endif
...
#else
//Push GPS sentences to buffer then decode it later with GPSParser()
#int_RDA HIGH
void RDA_isr()
{
	int ch;
...
//Set satellite signal to all 0xFF
void gps_ResetSatelliteSignal()
{
	int i;
	for(i=0; i<32; i++)
...
//Convert string at GPSBuffer+g_nGPSBufferReadPtr to float number
float gps_strtof()
{
...
//Convert string at GPSBuffer+g_nGPSBufferReadPtr to unsigned long number
long gps_strtoul()
{
	long lResult, lTemp;
	int nLen;
...
int parseGPRMC()
{
	...
	recGPSData.nSpeed = recGPSData.fSpeed;
	...
	if(GPSBuffer[g_nGPSBufferReadPtr] == '$')   //Valid GPS sentece?
	{
		...
		if(GPSBuffer[g_nGPSBufferReadPtr] == 'G')
		{
			...
			if(GPSBuffer[g_nGPSBufferReadPtr] == 'P')
			{
				...
				//Get 3 char sentence, e.g. 'GGA', 'RMC', 'GSA'
