...
btSetAddress(void)
{
  ...
  while(done == FALSE)
  {
    ...
    if (anyKey != KEY_NOTHING)
    {
      ...
      if (anyKey == KEY_CENTER)
      {
		static tMenuItem const items[] = {
			{"Cancel", NULL, NULL},
			{"Store new addr", NULL, NULL}};
		static tMenu menu = {"Exit", NULL, items, sizeof(items) / sizeof(tMenuItem), 0};
...
drawBtMenu(void)
{
  ...
  lcdRect(0, 0, 130, 130, BT_BACKBACKGROUND_COLOR);
  lcdRect(1, 16, 126, 84, BT_BACKGROUND_COLOR);
